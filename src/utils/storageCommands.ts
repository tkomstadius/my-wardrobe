...
function mapRowToItem(row: Record<string, unknown>): WardrobeItem {
  return {
    ...
    wearHistory: Array.isArray(row.wear_history) ? (row.wear_history as string[]).map((d) => new Date(d)) : [],
...
export async function loadItems(): Promise<WardrobeItem[]> {
  try {
    const userId = await getCurrentUserId()
    const { data, error } = await supabase.from('wardrobe_items').select(ITEM_COLUMNS).eq('user_id', userId)
...
export async function archiveItem(id: string, reason: ArchiveReason, notes?: string): Promise<void> {
  const userId = await getCurrentUserId()
...
export async function logWearOnDate(itemId: string, date: Date): Promise<void> {
  ...
  const currentHistory = Array.isArray(data.wear_history) ? (data.wear_history as string[]).map((d) => new Date(d)) : []
...
export async function getItemStoragePath(id: string): Promise<string | null> {
  const userId = await getCurrentUserId()
  const { data } = await supabase.from('wardrobe_items').select('image_url').eq('id', id).eq('user_id', userId).single()
...
export async function getOutfitById(id: string): Promise<Outfit | null> {
  try {
    const userId = await getCurrentUserId()
    const { data, error } = await supabase.from('outfits').select('*').eq('id', id).eq('user_id', userId).single()
...
export async function removeOutfit(id: string): Promise<void> {
  try {
    ...
    const { data } = await supabase.from('outfits').select('photo_url').eq('id', id).eq('user_id', userId).single()
...
export async function updateOutfit(id: string, updates: Partial<Outfit>): Promise<void> {
  ...
  const { error } = await supabase.from('outfits').update(updateRow).eq('id', id).eq('user_id', userId)
