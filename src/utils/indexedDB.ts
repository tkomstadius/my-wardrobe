...
export async function loadAllItems(): Promise<Array<WardrobeItem>> {
  ...
  const items = await Promise.all(dbItems.map((dbItem) => dbItemToWardrobeItem(dbItem)));

  // Clean up old trait data: if dbItem has trait field (from old data), remove it
  const itemsToClean = dbItems.filter((dbItem) => 'trait' in dbItem && dbItem.trait !== undefined);
...
export async function loadItemsByCategory(category: ItemCategory): Promise<WardrobeItem[]> {
  const db = await getDB();
  const tx = db.transaction(STORE_NAME, 'readonly');
...
async function dbOutfitToOutfit(dbOutfit: DBOutfit): Promise<Outfit> {
  ...
  return {
    id: dbOutfit.id,
    photo: dbOutfit.photoBlob ? await blobToDataURL(dbOutfit.photoBlob) : undefined,
...
export async function loadAllOutfits(): Promise<Outfit[]> {
  ...
  const outfits = await Promise.all(dbOutfits.map((dbOutfit) => dbOutfitToOutfit(dbOutfit)));
